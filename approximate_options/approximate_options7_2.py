...
x = generate_x(np.array([0, 0, 0]), np.array([100, 100, 100]), 200, 42)
x_validate = generate_x(
    np.array([0, 0, 0]), np.array([100, 100, 100]), 100, 41
)

approximate_options7_2 = [
    {
        ...
        "params": convert_variables([1 for x in range(61)]),
    ...
    },
    {
        ...
        "params": convert_variables([1 for x in range(61)]),
        "loss_function": tf.keras.losses.MSE,
        "opt": tf.keras.optimizers.SGD(learning_rate=10, momentum=0.5, nesterov=True),
        "eps": 0.0001,
        "max_steps": 300,
    },
    {
...
approximate_options7_2_white_noise = [
    copy.deepcopy(x) for x in approximate_options7_2
]
for x in approximate_options7_2_white_noise:
    x["noise_type"] = "white_noise"
    x["seed"] = 42

approximate_options7_2_gaussian_noise = [
    copy.deepcopy(x) for x in approximate_options7_2
]
for x in approximate_options7_2_gaussian_noise:
    x["noise_type"] = "gaussian_noise"
    x["seed"] = 42
    x["mean"] = 1
    x["std"] = 5

approximate_options7_2_salt_and_papper_noise = [
    copy.deepcopy(x) for x in approximate_options7_2
]
for x in approximate_options7_2_salt_and_papper_noise:
